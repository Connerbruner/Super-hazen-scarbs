    ...
    public void Decerease() {
        if (invincible <= 0) {
            if (getID() == 1) {
                ...
                game.pauseEnemys();
                game.AnimationGroup.add(new FallingDeadMario(getX(), getY(), game));
                game.playSound("music/smb_mariodie.mp3");
                game.bsMusic.stopAll();
            } else if (getID() == 2) {
                setID(1);
                ...
                game.parent.MarioStatus = 1;
                game.pauseEnemys();
                game.AnimationGroup.add(new FireToSmallMarioAnim(getX(), getY(), game.bsLoader.getStoredImages("FireToSmallMarioAnim"), game, RightActivated));
                invincible = 200;
                game.playSound("music/smb_pipe.mp3");
            }
        }
