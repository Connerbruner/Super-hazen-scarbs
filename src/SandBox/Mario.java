...
public class Mario extends GameObject {
    ...
    final float JOYSTICK_LIMIT = 0.9f;
    ...
    Component jStickX = controllerComponents[4];
    Component jStickY = controllerComponents[3];
    Component button1 = controllerComponents[17]; // last button slot (next to the joystick) ("Button 11")
    Component button2 = controllerComponents[16]; // second last button slot ("Button 10")
    Component button3 = controllerComponents[15]; // third last button slot ("Button 9")
    Component button4 = controllerComponents[14]; // fourth last button slot ("Button 8")
    ...
    public void update( long l ) {
        ...
        if ( PlayGame ) {
            ...
            if ( jStickX.getPollData( ) < -JOYSTICK_LIMIT ) {
                player.GoToRight( );
            }
            if ( jStickX.getPollData( ) > JOYSTICK_LIMIT ) {
                player.GoToLeft( );
            }
            if ( jStickY.getPollData( ) < -JOYSTICK_LIMIT  ) {
                player.Down( );
            } else {
                player.KeyPressedDown = false;
            }
            if ( button2.getPollData( ) > JOYSTICK_LIMIT  ) {
                player.Up( );
            } else {
            ...
            player.Speed( bsInput.isKeyDown( 90 ) );

            if ( bsInput.isKeyDown( 88 ) ) {
                player.Jump( );
            }
...
//                    checkpoint.setImage(this.bsLoader.getStoredImage("clowd_checkpoint.png")); // made visible for testing purpuse
    ...
    public void Restart( ) {
        parent.setCoin( parent.getCoin( ) - 5 );
        if ( parent.getCoin( ) <= 0 ) {
            parent.setCoin( 0 );
        }
        PlayEnemys( );
        player.setLocation( 9 , 3 );
        player.speed          = 0;
        PlayLivingThings      = true;
